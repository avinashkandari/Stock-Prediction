...
import { FiTrendingUp, FiAlertTriangle, FiCheckCircle, FiRefreshCw } from 'react-icons/fi';
...
function StockPredictor() {
  ...
  const formatISTDate = (dateString) => {
    ...
    // Add timezone offset for IST (UTC+5:30)
  ...
  useEffect(() => {
    if (predictionData) {
      console.log('Prediction base date:', predictionData.metrics.prediction_date);
      console.log('Forecast dates:', predictionData.forecast.dates);
    }
  }, [predictionData]);
  ...
  return (
    <div className="app-container">
      <header className="app-header">
        ...
        <p className="header-subtitle">AI-powered stock market forecasting (IST)</p>
      </header>
          ...
          <section className="status-card card">
            ...
            <div className="status-content">
              ...
              <div>
                ...
                <p>Epoch {Math.floor(progress / 10)} of 10</p>
                <small>Polling attempt: {pollCount}</small>
              </div>
            </div>
          </section>
        )}
        ...
        {status === 'ready' && predictionData && (
          <>
            ...
            <section className="forecast-card card">
              <h2>10-Day Forecast from {formatISTDate(predictionData.metrics.prediction_date)}</h2>
